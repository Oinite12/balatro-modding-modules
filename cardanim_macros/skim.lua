...
return function(macro_obj)
	...
	for layer_name, __ in pairs(G[mod_prefix .. "_cardanim_cfg"].card_layers) do
		...
		local iter_keys = {}
		for k, _ in pairs(active_area) do
			table.insert(iter_keys, k)
		end
		...
		for _, i in ipairs(iter_keys) do
			...
			local subiter_keys = {}
			for k, _ in pairs(active_area[i]) do
				table.insert(subiter_keys, k)
			end
